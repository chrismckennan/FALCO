...
#' @param partition.params A list of objects used to partition CpGs into independent units if analyzing DNA methylation data. The elements of the list are \code{chr} (a number of CpGs vector giving chromosome information; can be a character or numeric vector), \code{pos} (a number of CpGs vector giving the base pair position of each CpG), \code{size} (the size of the partition, in base pairs). The default is conditional on the latent factors, CpGs are independent (i.e. \code{size=1}). If specified, it is recommend the user set \code{size=1e5} to ensure the estimate for \code{K} is not corrupted by regional dependence.
...
CBCV_plus <- function(Y, Cov=NULL, maxK=20, B=NULL, nFolds=5, simpleDelta=T, partition.params=list(chr=NULL,pos=NULL,size=1), A.ine=NULL, c.ine=NULL, A.equ=NULL, Var.0=NULL, tol.rho=1e-3, max.iter.rho=15, svd.method="slow", plotit=T, n_cores=NULL) {
  ...
  if (is.null(B)) {
    return( ChooseK_NoB(Y=Y, X=Cov, maxK=maxK, nFolds=nFolds, simpleDelta=simpleDelta, partition.params=partition.params, max.iter.svd=3, svd.method=svd.method, plotit=plotit, n_cores=n_cores) )
  }
