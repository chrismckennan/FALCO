...
ChooseK_NoB <- function(Y, X=NULL, maxK, nFolds=10, simpleDelta=F, partition.params=list(chr=NULL,pos=NULL,size=1e6), max.iter.svd=3, svd.method="fast", plotit=T, n_cores=NULL) {
  if (maxK < 1) {
    return(0)
  }
...
my.cut <- function(x, nFolds) {
  out <- rep(NA,length(x))
  n <- floor(length(x)/nFolds)
  r <- length(x) - n*nFolds
  count <- 1
  for (i in 1:nFolds) {
    ind.stop <- count + n + ifelse(i<=r,1,0) - 1
    out[count:ind.stop] <- i
    count <- ind.stop + 1
  }
  return(out)
}
